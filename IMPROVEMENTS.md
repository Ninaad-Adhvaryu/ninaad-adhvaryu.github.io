...
## Homepage — secondary-list thumbnails — 2026-07-07
- **Added a hero thumbnail to every "more inquiries" item (all 17).** The flat text list read as too dry beside the image-bearing featured cards. Each `<li class="secondary-item">` now carries a 116×65 (16:9) `hero.svg` thumbnail inside its link (`loading="lazy" decoding="async"`, explicit dimensions, `alt=""` since the adjacent title is the link label). `.secondary-link` became a two-column grid (`auto 1fr`) with a new `.secondary-thumb` class; `.secondary-item` align changed `baseline` → `center`. Featured grid, `.athena-framing`, and status labels untouched; `git diff index.html` is scoped to the list + its CSS.
- **Created `mapping-mangroves/site/hero.svg`** (the one secondary essay that lacked a hero — its page uses data-figure PNGs). Original house-style, text-free: a wavy shoreline over a teal mangrove-detection patchwork, deep where detection is dense/agreed, fading at the disagreement fringes. New file, so its thumbnail now matches the other sixteen. (The essay page itself was not modified — the new hero is available there if you later want to add one.)
